 ...
 * @author STFIL
 **/
contract DistributionManager is Initializable, IDistributionManager {
  ...
  function __DistributionManager_init(address emissionManager) internal onlyInitializing {
    __DistributionManager_init_unchained(emissionManager);
  }

  function __DistributionManager_init_unchained(address emissionManager) internal onlyInitializing {
    ...
    returns (uint256)
  {
    ...
    for (uint256 i = 0; i < stakes.length; i++) {
      accruedRewards += _updateUserAssetInternal(
          user,
    ...
    returns (uint256)
  {
    ...
    for (uint256 i = 0; i < stakes.length; i++) {
      ...
      accruedRewards += _getRewards(stakes[i].stakedByUser, assetIndex, assetConfig.users[user]);
    }
    return accruedRewards;
  }
